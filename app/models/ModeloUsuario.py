...
class ModeloUsuario():
    ...
    def login(cls, db, usuario):
        """
        Verifica la existencia del usuario de manera vulnerable a inyección SQL.
        
        :param db: Objeto de conexión a la base de datos.
        :param usuario: Objeto con atributos `usuario` y `password`.
        :return: Un objeto Usuario si el usuario existe, de lo contrario None.
        """
        try:
            cursor = db.connection.cursor()
            ...
            if data:
                # Se omite la verificación de contraseña para la demostración
                usuario_logeado = Usuario(data[0], data[1], None, None)
                return usuario_logeado
            else:
                return None
